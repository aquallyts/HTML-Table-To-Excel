...
namespace WebApplication //altere o namespace de acordo com sua necessidade
{
    public class TableToExcel
    {
        ...
        {
            XmlDocument xml = new XmlDocument(); //aqui é substituida Classe WebBrowser
            xml.LoadXml(html);
            
            foreach (var table in xml.GetElementsByTagName("table")) //WebBrowser e XmlDocument possuiem o mesmo tipo de metodo
            {
                processTable(table);
            }
        ...
        private void processTable(dynamic table)
        {
            ...
            foreach (var row in table.GetElementsByTagName("tr"))
            {
                ...
                foreach (var td in tds)
                {
                    ...
                    rowSpan = getSpan(td.OuterXml, 0);// alterado de OuterHtml para OUterXml
